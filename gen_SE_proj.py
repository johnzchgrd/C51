...
multi_t_dd='''
  ...
  <project>
    <PathAndName>.\\sub_prjs\\SE%d_%d.uvproj</PathAndName>
...
'''
uvproj_s_dd_dd_d_dd_s='''<?xml version="1.0" encoding="UTF-8" standalone="no" ?>
    ...
    <Target>
      ...
      <Groups>
        <Group>
          ...
          <Files>
            <File>
              <FileName>se%d_%d.asm</FileName>
              <FileType>%d</FileType>
              <FilePath>..\\src\\se%d_%d.%s</FilePath>
            </File>
          </Files>
        </Group>
      </Groups>
...
<ProjectOpt xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="project_opt.xsd">
  ...
  <Group>
    ...
    <File>
      ...
      <tvExpOptDlg>0</tvExpOptDlg>
      <bDave2>0</bDave2>
      <PathWithFileName>..\\src\\se%d_%d.%s</PathWithFileName>
      <FilenameWithoutPath>se%d_%d.%s</FilenameWithoutPath>
      <RteFlg>0</RteFlg>
      <bShared>0</bShared>
    </File>
  </Group>
...
## open files
import os
root_dir="./SE"+str(SE_no)+"/"
os.mkdir(path=root_dir)
uvmpw = open(root_dir+"SE"+str(SE_no)+".uvmpw",mode="w")
...
for i in range(1,1+num_class):
    for j in range(1,1+num_subclass[i-1]):
        tmp=open(sub_dir+"SE%d_%d.uvproj"%(i,j),mode="w")
        tmp.write(uvproj_s_dd_dd_d_dd_s%(i,j,i,j,typelist[filetype],i,j,filetype))
        tmp.flush()
        tmp.close()
...
for i in range(1,1+num_class):
    for j in range(1,1+num_subclass[i-1]):
        tmp=open(sub_dir+"SE%d_%d.uvopt"%(i,j),mode="w")
        tmp.write(uvopt_s_dd_d_dds_dds%(i,j,typelist[filetype],i,j,filetype,i,j,filetype))
        tmp.flush()
        tmp.close()

## create empty src files
for i in range(1,1+num_class):
    for j in range(1,1+num_subclass[i-1]):
        tmp=open(src_dir+"se%d_%d.%s"%(i,j,filetype),mode="w")
        ## write nothing
        tmp.close()
